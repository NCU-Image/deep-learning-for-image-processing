...
def first_tpcnn_train(model_name,index):
    ...
    cp_callback_save_best = tf.keras.callbacks.ModelCheckpoint(filepath=checkpoint_save_best_path,monitor='val_sparse_categorical_accuracy',
                                                               save_weights_only=True, save_best_only=True,mode='max') # 用于保存最优权重
    csv_logger = CSVLogger(log_path,append=True)

    # 定义学习率调整器
    learningSchudel = LearningRateScheduler(step_decay)

    history = model.fit([x_I1_train, x_I2_train, x_I3_train, x_I4_train, x_I5_train], y_train, epochs=Epochs,initial_epoch=init_Epoch, batch_size=batch_size,validation_data=([x_I1_valid,x_I2_valid,x_I3_valid,x_I4_valid,x_I5_valid],y_valid),validation_freq=1, callbacks=[csv_logger,cp_callback_save,cp_callback_save_best])
