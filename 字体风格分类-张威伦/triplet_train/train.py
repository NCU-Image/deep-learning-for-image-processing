...
from triplet_net.facenet import Facenet
from triplet_net.facenet_training import (get_lr_scheduler, set_optimizer_lr,
                                          triplet_loss)
from triplet_util.dataloader import FacenetDataset, dataset_collate, TestDataSet
from triplet_util.utils import get_num_classes, show_config
from triplet_util.utils_fit import fit_one_epoch
...
if __name__ == "__main__":
    ...
    if True:
        ...
        epoch_step_val = len(lines_val) // (batch_size // 3)
        epoch_step_test = len(lines_test) // (batch_size // 3)
        ...
        #   构建数据集加载器。
        # ---------------------------------------#
        train_dataset = FacenetDataset(input_shape, lines, num_classes, random=True)
        ...
        for epoch in range(Init_Epoch, Epoch):
            ...
            fit_one_epoch(model_train, model, loss_history, loss, optimizer, epoch, epoch_step, epoch_step_val,
                          epoch_step_test, gen, gen_val, gen_test, Epoch, Cuda, batch_size // 3, save_period, save_dir)
