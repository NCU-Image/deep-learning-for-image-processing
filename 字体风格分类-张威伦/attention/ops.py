...
from attention.fca import MultiSpectralAttentionLayer
from attention.sasa import AttentionConv
    ...
    def __init__(self, pool_mode='avg', sizes=(1, 2, 3, 6)):
        ...
        if pool_mode == 'avg':
            ...
            self.pool_blocks = nn.ModuleList([
                nn.AvgPool2d(7, 7, padding=0),
                nn.AvgPool2d(4, 3, padding=0),
                nn.AvgPool2d(3, 3, padding=1)
            ])
        elif pool_mode == 'max':
            # pool_layer = nn.AdaptiveMaxPool2d.
            self.pool_blocks = nn.ModuleList([
                nn.MaxPool2d(7, 7, padding=0),
                nn.MaxPool2d(4, 3, padding=0),
        ...
        # self.pool_blocks = nn.ModuleList([
        #     nn.Sequential(pool_layer(size)) for size in sizes
        # ])

    def forward(self, x):
        xs = [block(x) for block in self.pool_blocks]
        xs = [xi.view(xi.size(0), -1) for xi in xs]
...
class AttentionModule(nn.Module):
    ...
    #     init_list = [self.spatial, self.channel]
    #     for i in init_list:
    #         for layer in i:
    ...
    def forward(self, x):
        ...
        x_style = (x * channel_scale + x * spatial_scale) * 0.5

        x_noise = x - x_style

        return x_style, x_noise
...
class SEBlock(nn.Module):
    def __init__(self, in_channels, reduction=16):
        ...
        self.fc_1 = nn.Linear(self.in_channels, self.in_channels // self.reduction, bias=True)
        self.fc_2 = nn.Linear(self.in_channels // self.reduction, self.in_channels, bias=True)
        self.act_1 = nn.ReLU()
        self.act_2 = nn.Sigmoid()

    def forward(self, x):
        y = self.global_avg_pooling(x)
        batch, featrues = y.size(0), y.size(1)
        y = y.view(batch, featrues)
        y = self.act_1(self.fc_1(y))
        y = self.act_2(self.fc_2(y))
        y = y.view(batch, featrues, 1, 1)
        return x * y
...
class AttentionDecomposition(nn.Module):
    def __init__(self, in_channels):
        super(AttentionDecomposition, self).__init__()
        self.attention = SEBlock(in_channels=in_channels)

    def forward(self, x):
        style = self.attention(x)
        noise = x - style
        return style, noise
